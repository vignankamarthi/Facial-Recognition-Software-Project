    ...
    try:
        ...
        if operation_type == "detect":
            ...
            for i, (top, right, bottom, left) in enumerate(face_locations):
                ...
                cv2.rectangle(
                    result_image, (left, top), (right, bottom), (0, 255, 0), 2
                )
                logger.debug(
                    f"Drew box for face {i+1} at location ({left}, {top}, {right}, {bottom})"
                )
...
def main():
    ...
    try:
        ...
        if not os.path.exists(test_image):
            ...
            cv2.ellipse(
                img, (200, 180), (60, 40), 0, 0, 180, (0, 255, 255), -1
            )  # Mouth
            # Save the image
        ...
        if success:
            print(
                f"  Image processing succeeded: {result.get('face_count')} faces found"
            )
    ...
    success, result = process_image_with_error_handling(
        os.path.join(test_dir, "test_image.jpg"), operation_type="fail"
    )
    if success:
