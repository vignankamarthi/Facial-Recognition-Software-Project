...
class TestFaceMatcher:
    ...
    def test_initialization(self):
        ...
        with patch('src.backend.face_matching.FaceMatcher.load_known_faces') as mock_load_faces:
            matcher = FaceMatcher()
    ...
    def test_empty_known_faces_dir(self, test_data_dir):
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            mock_print.assert_any_call("Please add reference face images to compare against")
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            # Verify warning message was printed
            mock_print.assert_any_call(f"Creating directory since existing one wasn't found: {nonexistent_dir}")
    ...
    def test_identify_faces(self, sample_image, mock_face_recognition):
        ...
        face_encodings = [np.ones(128), np.random.rand(128)]  # First should match John Smith
        ...
        with patch('face_recognition.compare_faces') as mock_compare, \
            ...
            mock_compare.side_effect = [[True, False], [False, False]]  # First face matches, second doesn't
            mock_distance.side_effect = [np.array([0.3, 0.9]), np.array([0.7, 0.8])]  # First face close to first known face
            ...
            assert "John Smith" in face_names[0]  # Should include name with confidence score
    ...
    def test_identify_faces_no_known_faces(self, sample_image):
        ...
        # Test data
        face_locations = [(50, 200, 150, 100), (200, 350, 300, 250)]
        face_encodings = [np.ones(128), np.zeros(128)]  # Won't be used since no known faces
    ...
    def test_match_faces_webcam(self, mock_cv2, mock_face_recognition):
        ...
        with patch('src.backend.face_matching._FaceDetector', create=True) as mock_detector_class, \
             patch('src.backend.face_matching.create_resizable_window') as mock_create_window, \
             patch('src.backend.face_matching.cv2.imshow') as mock_imshow, \
             patch('src.backend.face_matching.cv2.waitKey') as mock_waitkey, \
             patch('src.backend.face_matching.safely_close_windows') as mock_close_windows, \
             patch.object(matcher, 'identify_faces') as mock_identify:
            
            # Configure mocks
            mock_detector = MagicMock()
            mock_detector.detect_faces.return_value = ([(50, 200, 150, 100)], [np.ones(128)])
            mock_detector_class.return_value = mock_detector
            
            mock_waitkey.return_value = ord('q')  # Simulate pressing 'q' to quit
            
            mock_identify.return_value = (np.zeros((300, 400, 3), dtype=np.uint8), ["Test Person"])
        ...
        with patch('src.backend.face_matching.format_error') as mock_format_error, \
             patch('src.backend.face_matching.safely_close_windows') as mock_close_windows, \
             patch('builtins.print') as mock_print:
    ...
    def test_identify_faces_with_threshold(self):
        ...
        with patch('face_recognition.compare_faces') as mock_compare, \
             ...
             patch('cv2.rectangle') as mock_rectangle, \
             patch('cv2.putText') as mock_putText, \
             patch('src.backend.face_matching.FACE_MATCHING_THRESHOLD', 0.6):
            
            # Case 1: Face distance below threshold (should match)
            mock_compare.return_value = [True]
            mock_distance.return_value = np.array([0.3])  # 70% confidence, above threshold
            
            result_frame, face_names = matcher.identify_faces(test_image, face_locations, face_encodings)
            ...
            mock_distance.return_value = np.array([0.7])  # 30% confidence, below threshold
    ...
    def test_handle_opencv_error_decorator(self):
        ...
        with patch('src.backend.face_matching.safely_close_windows') as mock_close_windows:
