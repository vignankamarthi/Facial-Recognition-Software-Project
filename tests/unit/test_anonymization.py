...
class TestFaceAnonymizer:
    ...
    def test_anonymize_face_blur(self, sample_image):
        ...
        anonymizer = FaceAnonymizer(method="blur", intensity=21)  # Use odd intensity for kernel size
        ...
        with patch('cv2.GaussianBlur') as mock_blur, \
            ...
            blurred_face = np.zeros((100, 100, 3), dtype=np.uint8)  # Mock blurred result
    ...
    def test_anonymize_face_mask(self, sample_image):
        ...
        with patch('cv2.rectangle') as mock_rectangle, \
             ...
             patch('cv2.ellipse') as mock_ellipse, \
             patch('cv2.putText') as mock_text:
    ...
    def test_anonymize_frame(self, sample_image, mock_face_locations):
        ...
        with patch.object(anonymizer, 'anonymize_face') as mock_anonymize_face, \
            ...
            for i, face_location in enumerate(face_locations):
                mock_anonymize_face.assert_any_call(mock_anonymize_face.return_value, face_location)
    ...
    def test_demonstrate_methods(self, sample_image, mock_face_locations):
        ...
        with patch.object(anonymizer, 'anonymize_face') as mock_anonymize_face:
            # Configure the mock to return a copy of the image
            mock_anonymize_face.side_effect = lambda frame, loc, method=None: frame.copy()
    ...
    def test_run_anonymization_demo(self):
        ...
        with patch('src.backend.anonymization.FaceAnonymizer') as mock_anonymizer_class, \
             patch('src.backend.anonymization._FaceDetector', create=True) as mock_detector_class, \
             patch('cv2.VideoCapture') as mock_video_capture, \
             patch('src.backend.anonymization.create_resizable_window') as mock_create_window, \
             patch('cv2.imshow') as mock_imshow, \
             patch('cv2.waitKey') as mock_waitkey, \
             patch('src.backend.anonymization.safely_close_windows') as mock_close_windows:
            ...
            mock_video_capture.return_value.read.return_value = (True, np.zeros((300, 400, 3), dtype=np.uint8))
    ...
    def test_anonymize_face_with_even_intensity(self):
        ...
        with patch('cv2.GaussianBlur') as mock_blur:
            ...
            assert args[1] == (31, 31)  # Should be (intensity+1, intensity+1) since 30 is even
