...
class TestFaceDetector:
        ...
        with patch('cv2.rectangle') as mock_rectangle:
            ...
            for i, (top, right, bottom, left) in enumerate(face_locations):
                mock_rectangle.assert_any_call(
                    result_image, (left, top), (right, bottom), test_color, test_thickness
                )
    ...
    def test_detect_faces_webcam(self, mock_cv2):
        ...
        with patch('src.backend.face_detection.create_resizable_window') as mock_create_window, \
             patch('src.backend.face_detection.cv2.imshow') as mock_imshow, \
             patch('src.backend.face_detection.cv2.waitKey') as mock_waitkey, \
             patch('src.backend.face_detection.safely_close_windows') as mock_close_windows, \
             patch.object(detector, 'detect_faces') as mock_detect:
        ...
        with patch('src.backend.face_detection.format_error') as mock_format_error, \
             patch('src.backend.face_detection.safely_close_windows') as mock_close_windows:
            success, result = detector.detect_faces_webcam()
    ...
    def test_process_image(self, sample_image):
        ...
        with patch.object(detector, 'detect_faces') as mock_detect, \
             ...
             patch('src.backend.face_detection.cv2.imshow') as mock_imshow, \
             patch('src.backend.face_detection.cv2.waitKey') as mock_waitkey, \
             patch('src.backend.face_detection.cv2.destroyAllWindows') as mock_destroy:
    ...
    def test_detect_faces_exception_handling(self):
        ...
        with patch('src.backend.face_detection.face_recognition.face_locations') as mock_locations, \
             patch('src.backend.face_detection.log_exception') as mock_log_exception:
    ...
    def test_detect_faces_webcam_with_anonymization(self):
        ...
        with patch('src.backend.face_detection.create_resizable_window') as mock_create_window, \
            ...
            # Configure mocks behavior
            mock_waitkey.return_value = ord('q')  # Simulate pressing 'q' to quit after one frame
            mock_detect.return_value = ([(50, 350, 250, 50)], [np.ones(128)])
            
            # Call the method with anonymization
            success, result = detector.detect_faces_webcam(anonymize=True, anonymizer=mock_anonymizer)
