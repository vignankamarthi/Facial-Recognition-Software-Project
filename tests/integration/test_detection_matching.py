...
class TestDetectionMatchingIntegration:
    ...
    def test_webcam_matching_integration(self, detection_matching_pipeline):
        ...
        with patch("cv2.VideoCapture", return_value=mock_video_capture), patch.object(
            detector.__class__,
            "detect_faces",
            return_value=([(50, 200, 150, 100)], [np.ones(128)]),
        ), patch.object(matcher, "identify_faces") as mock_identify, patch(
            "cv2.imshow"
