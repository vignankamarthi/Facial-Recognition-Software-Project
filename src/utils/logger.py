...
def get_logger(name):
    ...
    Examples
    --------
    >>> # In file src/backend/face_detection.py
...
def log_exception(logger, message, exception=None, level=logging.ERROR):
    ...
    context_msg = (
        f"{message} in {function_name} at {Path(file_path).name}:{line_number}"
    )
...
def get_all_logs():
    ...
    """
    log_files = {"error": ERROR_LOG, "info": INFO_LOG, "debug": DEBUG_LOG}
