...
def process_matching_frame(frame):
    ...
    if hasattr(matcher, "set_threshold") and matching_params["threshold"] != config.matching.threshold:
    ...
    try:
        ...
        metadata = {
            ...
            "names": face_names if len(face_names) <= 10 else f"{len(face_names)} faces",
...
def face_matching_page():
    ...
    with main_tab:
        ...
        with st.expander("Matching Settings", expanded=False):
            updated_config = matching_config_panel(
                st.session_state.config["matching"],
                key_prefix="fm_"
            )
        ...
        with img_tab:
            ...
            st.markdown("Upload an image containing one or more faces to match against known references.")
    ...
    with manage_tab:
        ...
        known_faces_count = len([
            f for f in os.listdir(known_faces_dir) 
            if f.lower().endswith(('.jpg', '.jpeg', '.png'))
        ]) if os.path.exists(known_faces_dir) else 0
        
        if known_faces_count == 0:
            st.info("No known faces found. Add reference faces using the form below.")
        else:
            st.info(f"Found {known_faces_count} known faces. Select faces to manage them.")
        ...
        with col2:
            ...
            if selected_faces:
                ...
                remove_known_faces(known_faces_dir, selected_faces, key_prefix="fm_remove_")
            else:
    ...
    with st.expander("About Face Matching", expanded=False):
        ...
        - **Matching Threshold**: Controls how strict the matching is - higher values require closer matches
        - **Comparison Mode**: "Distance" measures difference (lower is better match), "Similarity" measures likeness (higher is better)
