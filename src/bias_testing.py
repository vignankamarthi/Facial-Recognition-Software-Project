...
class BiasAnalyzer:
    ...
    def test_recognition_accuracy(self, dataset_name):
        ...
        if results['overall']['total'] > 0:
            results['overall']['accuracy'] = results['overall']['detected'] / results['overall']['total']
    ...
    def visualize_results(self, dataset_name=None):
        ...
        accuracies = [results['by_demographic'][demo]['accuracy'] * 100 for demo in demographics]
    ...
    def run_bias_demonstration(self):
        ...
        if not os.path.exists(os.path.join(self.test_datasets_dir, 'sample_dataset')):
            ...
            print("Please add test images to the sample dataset directories, then run again.")
            return
